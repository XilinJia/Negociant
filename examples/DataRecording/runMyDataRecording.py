...
from checkData import checkNFillData
...
def runParentProcess(CTPFile='CTP_connect'):
    ...
    dataCheckTime = time(hour=17, minute=random.randint(1,59))
    ...
    while True:
        ...
        if datetime.today().weekday() < 5 and t.time() > dataCheckTime and (dataCheckDate is None or t.date() != dataCheckDate) :
            checkNFillData()
            dataCheckDate = t.date()
