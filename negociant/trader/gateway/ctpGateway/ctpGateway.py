...
from negociant.api.ctp import MdApi, TdApi, defineDict
from negociant.trader.vtGateway import *
from negociant.trader.vtFunction import getJsonPath, getTempPath
from negociant.trader.vtConstant import GATEWAYTYPE_FUTURES
...
class CtpAccount(object) :

    def __init__(self, fileName='CTP_connect') :
        ...
        self.brokerID = str(setting['brokerID'])
        ...
        if 'password' in setting :
            ...
            self.password = getpass.getpass("Enter your CTP password: ")

        # 如果json文件提供了验证码
        if 'authCode' in setting: 
            self.authCode = str(setting['authCode'])
            self.userProductInfo = str(setting['userProductInfo'])

        print("CTP account setting read")
...
class CtpGateway(VtGateway):
    ...
    #----------------------------------------------------------------------
    def __init__(self, eventEngine, gatewayName='CTP', account=None):
        """Constructor"""
        ...
        self.ctpAccount = account  
        
    #----------------------------------------------------------------------
    def connect(self):
        """连接"""
        
        if not self.ctpAccount :
            self.ctpAccount = CtpAccount()

        if self.ctpAccount.authCode :
            self.tdApi.requireAuthentication = True

        # 创建行情和交易接口对象
        self.mdApi.connect(self.ctpAccount.userID, self.ctpAccount.password, self.ctpAccount.brokerID, 
                            self.ctpAccount.mdAddress)
        self.tdApi.connect(self.ctpAccount.userID, self.ctpAccount.password, self.ctpAccount.brokerID, 
                            self.ctpAccount.tdAddress, self.ctpAccount.authCode, self.ctpAccount.userProductInfo)
