...
from negociant.trader.app.ctaStrategy.lkCtaTemplate import LKCtaTemplate
...
class MyKKStrategy(LKCtaTemplate):
    ...
    # 参数列表，保存了参数的名称
    paramList = LKCtaTemplate.paramList + ['kkLength', 'kkDev']    

    # 变量列表，保存了变量的名称
    varList = LKCtaTemplate.varList + ['kkUp', 'kkDown']
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(MyKKStrategy, self).__init__(ctaEngine, setting)
        
        self.barGen.requireXMinBar(5, self.onFiveBar)
    ...
    def onInit(self):
        ...
        # for bar in initData:
        #     print("onInit: ", bar.datetime, self.name, self.vtSymbol, " ", bar.close, '\n')
        #     self.onBar(bar)
    ...
    def onFiveBar(self, bar):
        ...
        self.logFile.write("onFiveBar: " + self.vtSymbol + ' ' + str(bar.time) + ' ' + str(self.pos) + ' ' + 
            str(bar.close) + ' ' + str(int(self.kkUp)) + ' ' + str(int(self.kkDown)) + '\n')
        self.logFile.flush()
        ...
        if self.pos == 0:
            ...
            self.fixedSize = self.getLots(bar.high)        
        ...
        # 持有多头仓位
            ...
            self.intraTradeLow = bar.low
            self.logFile.write("long cover price: " + str(self.intraTradeHigh*(1-self.trailingPrcnt/100)) + '\n')
        ...
        # 持有空头仓位
            ...
            self.logFile.write("short cover price: " + str(self.intraTradeLow*(1+self.trailingPrcnt/100)) + '\n')
